    ...
    for jj = 1:size(DEC,2)
        ...
        if (len_id > 3)
            ...
            if(length(centroid_newer) > 2)
                ...
                SPD_angles = dlmread('..\SPD\SPD_angles_Mv_6.txt');
                ...
                for i = 1 : length(SPD)
                    ...
                    for q = 1 : length(angle_final_list)
                        ...
                        for j = 2 : length(SPD(i,:))
                            if(SPD(i,j) ~= 0)
                                diff_angle = abs(angle_final_list(q) - SPD(i,j));
                                diff_angle_list = [diff_angle_list diff_angle];
                            end
                        end
                        min_diff_angle_list = min(diff_angle_list);
                        if(min_diff_angle_list < 0.075)
                            angle_matched_image = angle_final_list(q);
                            ind_angle_matched_image = q;
                            ind_SPD = find(diff_angle_list == min_diff_angle_list);
                            ind_SPD = ind_SPD + 1;
                            PSID = i;
                            break;
                        end
                    ...
                    for m = 2 : length(angle_final_list_new)
                        ...
                        if((SPD_new(ind_to_comp) - 0.25) < angle_final_list_new(m) && angle_final_list_new(m) < (SPD_new(ind_to_comp) + 0.25))
                            ...
                            for n = (ind_to_comp + 1) : length(SPD_new)
                                ...
                                if((add_SPD_angles - 0.35) < angle_final_list_new(m) && angle_final_list_new(m) < (add_SPD_angles + 0.35))
                                    ind_to_comp = n + 1;
                                    no_matches = no_matches + 1;
                                    break;
                                end
